...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    if (!alumniId || (action !== 'verify' && action !== 'unverify')) {
    ...
    try {
      ...
      console.error('Verify alumni ObjectId fallback:', error);
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: action === 'verify' ? 'Alumni verified successfully' : 'Alumni verification removed successfully',
      }),
      {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
